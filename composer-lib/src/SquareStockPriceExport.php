...
/**
 * Square Stock/Price Export
 *
 * Outputs a compact CSV for syncing stock and prices.
 * Columns: stock_id (SKU), Index (Square Token), Quantity on Hand, Price.
 */
class SquareStockPriceExport extends BaseCatalogueGenerator implements OutputHandlerInterface
{
    public function __construct($prefs_tablename)
    {
        parent::__construct($prefs_tablename);
        $this->filename = 'square_stock_price.csv';
    ...
    {
        return [
            'name' => 'square_stock_price',
            'title' => 'Square Stock + Price',
            'class' => 'SquareStockPriceExport',
            'description' => 'Export SKU + Square token + quantity on hand + price',
            'method' => 'createSquareStockPriceExport',
            'category' => 'ecommerce',
            'output_type' => 'csv',
            'version' => '1.0.0',
            'author' => 'KS Fraser',
    ...
    public function generateOutput()
    {
        try {
            ...
            return [
                ...
                'rows' => $rowcount,
                'files' => [$this->filename],
                'message' => "Successfully generated Square stock/price export with {$rowcount} rows",
            ];
        } catch (\Exception $e) {
            return [
                'success' => false,
    ...
    public function getStatus()
    {
        ...
        return 'Ready to export Square stock/price';
    }
    ...
    public function setQuery()
    {
        ...
        WHERE sm.inactive = 0
          AND st.square_token IS NOT NULL
          AND st.square_token <> ''
    ...
    public function createFile()
    {
        ...
        $result = $this->getDatabase()->query($this->query, "Couldn't export Square stock/price");
        $rowcount = 0;
        ...
        if ($rowcount > 0) {
            $this->emailFile('Square Stock + Price');
        }
        return $rowcount;
    }
}
